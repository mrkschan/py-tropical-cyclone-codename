...
tropic_cache = []
tropicurl = 'http://www.hko.gov.hk/prtver/html/docs/informtc/sound/tc_pronunciation2008e.shtml'

adj_cache = []
adjurl = 'http://www.keepandshare.com/doc/12894/adjective-list?p=y'

def get():
    if len(tropic_cache) == 0: build_cache()
    return '%s %s' % (random.choice(adj_cache), random.choice(tropic_cache))
    
def build_cache():
    f = urllib2.urlopen(tropicurl)
    for line in f.readlines():
        if p_tropic_head.search(line) is not None and p_tropic_tail.search(line) is None:
    ...
    for line in f.readlines():
        if p_adj_head.search(line) is not None: 
            m = p_adj_get.search(line)
            try:
                m = m.group(1)
                if p_adj_skip.search(m) is None:
